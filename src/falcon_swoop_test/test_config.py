...
def test_config_error_for_duplicate_operations() -> None:
    ...
    with pytest.raises(SwoopConfigError, match="Multiple functions are defined as GET"):
        Resource("/dummy")
...
def test_config_error_for_missing_operations() -> None:
    ...
    with pytest.raises(SwoopConfigError, match="Found no operation"):
        Resource("/dummy")
...
def test_config_error_for_duplicate_path_params_in_route() -> None:
    ...
    with pytest.raises(SwoopConfigError, match="Duplicate parameters were found in route"):
...
def test_config_error_for_mismatching_path_parameters() -> None:
    ...
    with pytest.raises(SwoopConfigError, match=exp_msg):
...
def test_config_error_for_mismatching_path_parameter_with_alias() -> None:
    ...
    with pytest.raises(SwoopConfigError):
        Resource(route)
...
def test_config_error_for_complex_param_type() -> None:
    with pytest.raises(SwoopConfigError, match="Query parameter query_params has unsupported type annotation"):
...
def test_config_error_for_missing_param_type() -> None:
    with pytest.raises(SwoopConfigError, match="Query parameter param requires type annotation"):
...
def test_config_error_for_optional_return_value() -> None:
    with pytest.raises(SwoopConfigError, match="Return type cannot be a union or optional"):
...
def test_config_error_for_optional_path_parameter() -> None:
    with pytest.raises(SwoopConfigError, match="Path parameter resource_id cannot be optional"):
...
def test_config_warning_for_optional_parameter_with_default() -> None:
    with pytest.warns(
        SwoopConfigWarning, match="Query parameter max_size is type hinted as optional, but will never be None"
    ):
...
def test_config_error_for_multiple_contexts() -> None:
    with pytest.raises(SwoopConfigError, match="Duplicated context parameter"):
...
def test_config_error_for_async_binary_input_on_sync_operation() -> None:
    with pytest.raises(SwoopConfigError, match="Operation is sync, but input type is configured as"):
...
def test_config_error_for_async_binary_output_on_sync_operation() -> None:
    with pytest.raises(SwoopConfigError, match="Operation is sync, but return type is configured as"):
...
def test_config_error_for_binary_input_on_async_operation() -> None:
    with pytest.raises(SwoopConfigError, match="Operation is async, but input type is configured as"):
...
def test_config_error_for_binary_output_on_async_operation() -> None:
    with pytest.raises(SwoopConfigError, match="Operation is async, but return type is configured as"):
